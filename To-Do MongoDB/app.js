...
app.get("/tasks",async (req,res)=>{
    ...
    try{
        ...
        console.log("Something went wrong during reading file from database");
    }
...
app.post("/tasks",async (req,res)=>{
    ...
    try{
        const todos = await TodoModel.find();
        console.log("database length : ",todos.length);

        const newTaskId = todos.length > 0 ? todos[todos.length-1].todo_id+1 : 1;
        ...
        console.log("Error in adding new task : ",error.message);
    }
...
app.put("/tasks/:id",async (req,res)=>{
    ...
    try {
        ...
        await TodoModel.updateOne({id:taskId},{completed:completed});
        const task = await TodoModel.find({taskId});
...
app.delete("/tasks/:id",async (req,res)=>{
    ...
    try {
        const taskId = parseInt(req.params.id);
        const taskDelete = await TodoModel.findOne({id:taskId});

        await TodoModel.deleteOne({id:taskId});
        res.json({message:"Task deleted successfully.",taskId});
        // give taskId and message back

    } catch (error) {
        console.log("Deleting task from the database : ",error.message);
    }
